...
class NetworkSimulation(QMainWindow):
    ...
    def change_to_red(self):
        ...
        # Print "Request forwarded to router 2" to the text output box
        self.text_output.append("Request forwarded by PIT")

        # Create a timer to change the color of the line connecting router 1 and base station to violet after 2 seconds
        timer2 = QTimer(self)
        timer2.timeout.connect(self.change_to_violet)
        timer2.setSingleShot(True)
        timer2.start(5000)

    def change_to_violet(self):
        # Change the color of the line connecting router 1 and base station to violet
        pen = QPen(Qt.magenta, 3)
        self.router_base_lines[0].setPen(pen)  # Change only the line between router 1 and base station

        # Print "Request forwarded to base station" to the text output box
        self.text_output.append("Request received by router 1 ")

        # Create a timer to change the color of the line connecting the user and router 1 to blue after 2 seconds
        timer3 = QTimer(self)
        timer3.timeout.connect(self.change_to_blue)
        timer3.setSingleShot(True)
        timer3.start(5000)

    def change_to_blue(self):
        # Change the color of the line connecting the user and router 1 to blue
        pen = QPen(Qt.blue, 3)
        self.user_router_line.setPen(pen)

        # Print "Request received by Router 1" to the text output box
        self.text_output.append("Request received by user ")
